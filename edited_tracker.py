    ...
    def validate_trackers(self, frame):
            ...
            for (t, l, c) in zip(self.trackers, self.labels, self.lives):
                ...
                if confidence <= 0.3:
                    ...
                    self.lives[index] = self.lives[index] + \
                        (1-confidence) if self.lives[index] < 3 else 3
                index += 1

    def search_and_match(self, boundries):
        ...
        for t in self.trackers:
            ...
            startX, startY = int(pos.left()), int(pos.top())
            endX, endY = int(pos.right()), int(pos.bottom())
            temp = intersectionOverUnion(boundries, (startX, startY, endX, endY))
            if temp > maxintersectionOverUnion:
                maxintersectionOverUnion = temp
                target = index
        #print(minError)
        if maxintersectionOverUnion > 0.2:
            return target
        else:
            return -1
        ...
        if self.detection_counter == 0:
            ...
            for i in np.arange(0, detections.shape[2]):
                ...
                if confidence > self.confidence:
                    ...
                    match = self.search_and_match((startX, startY, endX, endY))
                    ...
                    if match != -1:
                        temp = self.trackers[match]
                        self.trackers[match] = t
                        del temp
                        continue
